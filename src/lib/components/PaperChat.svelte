<script lang="ts">
	...
	import type { ChatMessage, ChatSummary, Paper } from '$lib/types';
	import {
		appendPaperChat,
		listPaperChats,
		newPaperChatId,
	...
	function threadSlug(): string {
		return paper.threadId ?? INBOX_SLUG;
	}

	async function loadChatList() {
		chats = await listPaperChats(threadSlug(), paper.arxivId);
	}

	async function openChat(id: string) {
		currentChatId = id;
		messages = await readPaperChat(threadSlug(), paper.arxivId, id);
		historyOpen = false;
		await tick();
		scrollToBottom();
	}
	...
	async function handleEvent(event: ChatEvent) {
		if (event.type === 'stdout') {
			...
			if (finalText) {
				...
				await appendPaperChat(threadSlug(), paper.arxivId, currentChatId, msg);
				await loadChatList();
			}
			streamBlocks = [];
			await tick();
	...
	async function send() {
		...
		composing = '';
		if (!currentChatId) currentChatId = newPaperChatId();
		await appendPaperChat(threadSlug(), paper.arxivId, currentChatId, userMsg);
		await loadChatList();
		await tick();
		scrollToBottom();
		...
		const workspaceDir = await threadDir(threadSlug());
...
<style>
	/* Compact markdown styles for chat bubbles. Mirrors the patterns in
	   PaperSummary's .summary-md but tuned for the smaller, denser chat view. */
